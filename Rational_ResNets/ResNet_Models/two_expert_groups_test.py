...
class RationalBasicBlock(nn.Module):
    ...
    def __init__(self, planes_in, planes_out, num_rationals: int = 4, stride=1, downsample=False):
        ...
        self.rational_6 = Rational(cuda=cuda, approx_func="leaky_relu")
        self.rational_7 = Rational(cuda=cuda, approx_func="sigmoid")
        self.rational_8 = Rational(cuda=cuda, approx_func="gelu")
        self.rational_9 = Rational(cuda=cuda, approx_func="tanh")
        self.rational_10 = Rational(cuda=cuda, approx_func="swish")
        data_2 = initialize_alpha(5)
        self.alpha_2 = torch.nn.parameter.Parameter(data_2, requires_grad=True)
    ...
    def multi_rational_2(self, out: Tensor) -> Tensor:
        rat_out_1 = self.rational_6(out.clone())
        rat_out_2 = self.rational_7(out.clone())
        rat_out_3 = self.rational_8(out.clone())
        rat_out_4 = self.rational_9(out.clone())
        rat_out_5 = self.rational_10(out.clone())

        out = self.alpha_2[0].clone() * rat_out_1.clone() + self.alpha_2[1].clone() * rat_out_2.clone() + self.alpha_2[2].clone() * rat_out_3.clone() + self.alpha_2[3].clone() * rat_out_4.clone() + self.alpha_2[4].clone() * rat_out_5.clone()
        return out
