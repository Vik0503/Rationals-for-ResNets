...
def _resnet(arch: str, block: Type[RationalBasicBlock], layers: List[int], num_rationals: int, **kwargs: Any) -> RationalResNet:
    ...
    """
    model = RationalResNet(block, layers, num_rationals=num_rationals, **kwargs)

    return model


def multi_select_variant_rational_resnet20(num_rationals: int = 4, **kwargs: Any) -> RationalResNet:
