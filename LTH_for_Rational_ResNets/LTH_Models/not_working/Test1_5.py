...
class RationalBasicBlock(nn.Module):
    ...
    def multi_variant_rationals(self, out) -> Tensor:
        ...
        temp_out = torch.flatten(out, start_dim=0, end_dim=1)
        r_tensor = self.r_rational(temp_out[0::3].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        g_tensor = self.g_rational(temp_out[1::3].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        b_tensor = self.b_rational(temp_out[2::3].clone().reshape(batch_size, rat_planes, img_shape, img_shape))

        r_tensor = torch.flatten(r_tensor, start_dim=0, end_dim=1)
        g_tensor = torch.flatten(g_tensor, start_dim=0, end_dim=1)
        b_tensor = torch.flatten(b_tensor, start_dim=0, end_dim=1)

        temp_out = temp_out.reshape(batch_size * num_planes, img_shape, img_shape).to(device)
        counter = 0
        counter_2 = 0
        for i in range(temp_out.shape[0]):
            if counter == 0:
                temp_out[i] = r_tensor[counter_2].clone()
                counter += 1
            elif counter == 1:
                temp_out[i] = g_tensor[counter_2].clone()
                counter += 1
            elif counter == 2:
                temp_out[i] = b_tensor[counter_2].clone()
                counter += 1
                counter = 0

        temp_out = temp_out.reshape(original_shape)
        out = temp_out
...
class RationalResNet(nn.Module):
    ...
    def multi_variant_rationals(self, out) -> Tensor:
        ...
        temp_out = out.clone()
        temp_out = torch.flatten(temp_out, start_dim=0, end_dim=1)
        r_tensor = self.r_rational(temp_out[0::3].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        g_tensor = self.g_rational(temp_out[1::3].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        b_tensor = self.b_rational(temp_out[2::3].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
