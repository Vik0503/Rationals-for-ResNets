...
class RationalBasicBlock(nn.Module):
    ...
    def __init__(self, planes_in: int, planes_out: int, stride: int = 1, downsample: bool = False):
        ...
        self.a_rational = Rational(cuda=cuda)
        self.b_rational = Rational(cuda=cuda)
        self.c_rational = Rational(cuda=cuda)
        self.d_rational = Rational(cuda=cuda)
    ...
    def multi_variant_rationals(self, out) -> Tensor:
        ...
        a_tensor = self.a_rational(out[0::4].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        b_tensor = self.b_rational(out[1::4].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        c_tensor = self.c_rational(out[2::4].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        d_tensor = self.d_rational(out[3::4].clone().reshape(batch_size, rat_planes, img_shape, img_shape))
        ...
        for i in range(out.shape[0]):
            if counter == 0:
                out[i] = a_tensor[counter_2]
                counter += 1
            elif counter == 1:
                out[i] = b_tensor[counter_2]
                counter += 1
            elif counter == 2:
                out[i] = c_tensor[counter_2]
                counter += 1
            elif counter == 3:
                out[i] = d_tensor[counter_2]
                counter_2 += 1
                counter = 0
...
class RationalResNet(nn.Module):
    ...
    def multi_variant_rationals(self, out) -> Tensor:
        ...
        r_tensor = torch.zeros(batch_size, rat_planes, img_shape, img_shape)
        g_tensor = torch.zeros(batch_size, rat_planes, img_shape, img_shape)
        b_tensor = torch.zeros(batch_size, rat_planes, img_shape, img_shape)
        c_tensor = torch.zeros(batch_size, rat_planes, img_shape, img_shape)

        for i in range(out.shape[0]):
            r_tensor[i] = out[i][0::4].clone().to(device)
            g_tensor[i] = out[i][1::4].clone().to(device)
            b_tensor[i] = out[i][2::4].clone().to(device)
            c_tensor[i] = out[i][3::4].clone().to(device)
...
model = rational_resnet20().to(device)
...
torch.save(model, '../../Saved_Models/model11.pth')