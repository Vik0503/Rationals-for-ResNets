...
def multi_variant_rationals(out: Tensor) -> Tensor:
    ...
    out_len = int(len(out) / 4)
    out_shape = out.shape[0]

    a_tensor = torch.zeros((batch_size * rat_planes, img_shape, img_shape)).to(device)
    b_tensor = torch.zeros((batch_size * rat_planes, img_shape, img_shape)).to(device)
    c_tensor = torch.zeros((batch_size * rat_planes, img_shape, img_shape)).to(device)
    d_tensor = torch.zeros((batch_size * rat_planes, img_shape, img_shape)).to(device)
    counter = 0
    counter_2 = 0
    for i in range(out.shape[0]):
        if counter == 0:
            a_tensor[counter_2] = out[i]
            counter += 1
        elif counter == 1:
            b_tensor[counter_2] = out[i]
            counter += 1
        elif counter == 2:
            c_tensor[counter_2] = out[i]
            counter += 1
        elif counter == 3:
            d_tensor[counter_2] = out[i]
            counter = 0
            counter_2 += 1

    a_tensor = rational(a_tensor.reshape(batch_size, rat_planes, img_shape, img_shape))
    b_tensor = rational(b_tensor.reshape(batch_size, rat_planes, img_shape, img_shape))
    c_tensor = rational(c_tensor.reshape(batch_size, rat_planes, img_shape, img_shape))
    d_tensor = rational(d_tensor.reshape(batch_size, rat_planes, img_shape, img_shape))

    """

    a_tensor = rational((out[0::4].clone().detach().to(device)).reshape(batch_size, int(num_planes/4), img_shape, img_shape)).reshape(batch_size * int(num_planes/4), img_shape, img_shape)
    b_tensor = rational((out[1::4].clone().detach().to(device)).reshape(batch_size, int(num_planes/4), img_shape, img_shape)).reshape(batch_size * int(num_planes/4), img_shape, img_shape)
    c_tensor = rational((out[2::4].clone().detach().to(device)).reshape(batch_size, int(num_planes/4), img_shape, img_shape)).reshape(batch_size * int(num_planes/4), img_shape, img_shape)
    d_tensor = rational((out[3::4].clone().detach().to(device)).reshape(batch_size, int(num_planes/4), img_shape, img_shape)).reshape(batch_size * int(num_planes/4), img_shape, img_shape)
"""
    ...
    # out = out.reshape(batch_size * 4, int(num_planes / 4), img_shape, img_shape)
    out = torch.zeros(batch_size * num_planes, img_shape, img_shape)
    counter = 0
    counter_2 = 0
    for i in range(out.shape[0]):
        if counter == 0:
...
writer = SummaryWriter('runs/multi_variant_v5_rational_resnet18_SVHN_run3')
...
def final_plot():
    plt.savefig('summary_plot_multi_rational_resnet_SVHN_6.svg')
    plt.show()
