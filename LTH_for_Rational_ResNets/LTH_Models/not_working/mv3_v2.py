...
class RationalBasicBlock(nn.Module):
    ...
    def __init__(self, stride=1, downsample=False):
        ...
        self.rgb_rational = Rational(cuda=cuda)
    ...
    def multi_variant_rationals(self, out) -> Tensor:
        ...
        r_tensor = out[0::3].clone().detach().to(device)
        r_tensor = r_tensor.reshape(batch_size, rat_planes, img_shape, img_shape)
        r_out = self.r_rational(r_tensor)

        g_tensor = out[1::3].clone().detach().to(device)
        g_tensor = g_tensor.reshape(batch_size, rat_planes, img_shape, img_shape)
        g_out = self.g_rational(g_tensor)

        b_tensor = out[2::3].clone().detach().to(device)
        b_tensor = b_tensor.reshape(batch_size, rat_planes, img_shape, img_shape)
        b_out = self.b_rational(b_tensor)

        r_out = torch.flatten(r_out, start_dim=0, end_dim=1)
        g_out = torch.flatten(g_out, start_dim=0, end_dim=1)
        b_out = torch.flatten(b_out, start_dim=0, end_dim=1)
