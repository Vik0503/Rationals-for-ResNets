...
def multi_variant_rationals(out: Tensor) -> Tensor:
    ...
    a_tensor = out[0::4].clone().detach().to(device)
    print(a_tensor)
    b_tensor = out[1::4].clone().detach().to(device)
    c_tensor = out[2::4].clone().detach().to(device)
    d_tensor = out[3::4].clone().detach().to(device)

    a_out = rational(a_tensor)
    b_out = rational(b_tensor)
    c_out = rational(c_tensor)
    d_out = rational(d_tensor)
    out = out.reshape(batch_size * int(num_planes / 4), 4, img_shape, img_shape)
    counter = 0
    for i in range(out_len):
        out[i][counter] = a_out[i].clone().detach()
        counter += 1
        out[i][counter] = b_out[i].clone().detach()
        counter += 1
        out[i][counter] = c_out[i].clone().detach()
        counter += 1
        out[i][counter] = d_out[i].clone().detach()
        counter = 0
...
writer = SummaryWriter('runs/multi_variant_v2_rational_resnet18_SVHN_run0')
...
def final_plot():
    plt.savefig('summary_plot_multi_rational_resnet_SVHN_4.svg')
    plt.show()
