...
class RationalBasicBlock(nn.Module):
    ...
    def __init__(self, stride=1, downsample=False):
        ...
        if downsample:
            self.shortcut = nn.Sequential(
                nn.Conv2d(3, self.expansion * 3, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(self.expansion * 3)
            )
...
def multi_variant_rationals(out) -> Tensor:
    r_rational = Rational(cuda=cuda)
    g_rational = Rational(cuda=cuda)
    b_rational = Rational(cuda=cuda)
    ...
    out = torch.flatten(out, start_dim=0, end_dim=1).requires_grad_(True)

    r_tensor = out[0::3].clone().detach().requires_grad_(True).to(device)
    r_tensor = r_tensor.reshape(batch_size, rat_planes, img_shape, img_shape)
    r_out = r_rational(r_tensor).requires_grad_(True)

    g_tensor = out[1::3].clone().detach().requires_grad_(True).to(device)
    g_tensor = g_tensor.reshape(batch_size, rat_planes, img_shape, img_shape)
    g_out = g_rational(g_tensor).requires_grad_(True)

    b_tensor = out[2::3].clone().detach().requires_grad_(True).to(device)
    b_tensor = b_tensor.reshape(batch_size, rat_planes, img_shape, img_shape)
    b_out = b_rational(b_tensor).requires_grad_(True)

    r_out = torch.flatten(r_out, start_dim=0, end_dim=1).requires_grad_(True)
    g_out = torch.flatten(g_out, start_dim=0, end_dim=1).requires_grad_(True)
    b_out = torch.flatten(b_out, start_dim=0, end_dim=1).requires_grad_(True)

    counter = 0
    counter_2 = 0
    out = out.reshape(batch_size * num_planes, img_shape, img_shape)
    for i in range(out.shape[0]):
        if counter == 0:
            out[i] = r_out[counter_2].clone().detach()
            counter += 1
        elif counter == 1:
            out[i] = g_out[counter_2].clone().detach()
            counter += 1
        elif counter == 2:
            out[i] = b_out[counter_2].clone().detach()
            counter_2 += 1
            counter = 0

    out = out.reshape(original_shape).requires_grad_(True)

    return out


def multi_variant_rationals_2(out) -> Tensor:
    r_rational = Rational(cuda=cuda)
    g_rational = Rational(cuda=cuda)
    b_rational = Rational(cuda=cuda)
    original_shape = out.shape
...
def final_plot():
    plt.savefig('summary_plot_multi_rational_resnet_SVHN_2.svg')
    plt.show()


model = multi_variant_rational_resnet20()
num_ftrs = model.fc.in_features
# Here the size of each output sample is set to nn.Linear(num_ftrs, len(class_names)).
model.fc = nn.Linear(num_ftrs, num_classes)

MVRRN18_model = model.to(device)

criterion = nn.CrossEntropyLoss()

# Observe that all parameters are being optimized
optimizer = optim.SGD(MVRRN18_model.parameters(), lr=0.01, momentum=0.9)
...
torch.save(MVRRN18_model, '../../Saved_Models/model11.pth')
