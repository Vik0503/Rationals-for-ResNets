...
def multi_variant_rationals(out: Tensor) -> Tensor:
    ...
    out = out.reshape(batch_size * int(num_planes / 4), 4, img_shape, img_shape)
    counter = 0
    for i in range(out_len):
        out[i][counter] = a_tensor[i].clone().detach()
        counter += 1
        out[i][counter] = b_tensor[i].clone().detach()
        counter += 1
        out[i][counter] = c_tensor[i].clone().detach()
        counter += 1
        out[i][counter] = d_tensor[i].clone().detach()
        counter = 0
...
writer = SummaryWriter('runs/multi_variant_v2_rational_resnet18_SVHN_run2')
...
def final_plot():
    plt.savefig('summary_plot_multi_rational_resnet_SVHN_5.svg')
    plt.show()
